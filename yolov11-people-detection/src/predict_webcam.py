...
class WebcamPredictor:
    ...
    def _initialize_camera(self):
        ...
        try:
            # Try several backends on Windows to improve compatibility (e.g. DSHOW, MSMF)
            backends = []
            # OpenCV backend constants may not exist on every build; guard with getattr
            if sys.platform.startswith('win'):
                if hasattr(cv2, 'CAP_DSHOW'):
                    backends.append(cv2.CAP_DSHOW)
                if hasattr(cv2, 'CAP_MSMF'):
                    backends.append(cv2.CAP_MSMF)
            # Always try the default backend last (None)
            ...
            for backend in backends:
                if backend is None:
                    logger.info("Trying default VideoCapture backend...")
                    cap = cv2.VideoCapture(self.camera_id)
                else:
                    logger.info(f"Trying VideoCapture backend {backend}...")
